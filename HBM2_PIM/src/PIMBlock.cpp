...
void PIMBlock::add(BurstType& dstBst, BurstType& src0Bst, BurstType& src1Bst)
{
    ...
    else if (pimPrecision_ == INT8)
    {
        for (int i = 0; i < 32; i++)
        {
            dstBst.u8Data_[i] = (uint8_t)((int8_t)src0Bst.u8Data_[i] + (int8_t)src1Bst.u8Data_[i]);
        }
    }
    else
...
void PIMBlock::mac(BurstType& dstBst, BurstType& src0Bst, BurstType& src1Bst)
{
    ...
    else if (pimPrecision_ == INT8)
    {
        for (int i = 0; i < 32; i++)
        {
            dstBst.u8Data_[i] = (uint8_t)((int8_t)src0Bst.u8Data_[i] * (int8_t)src1Bst.u8Data_[i] + (int8_t)dstBst.u8Data_[i]);
        }
    }
    else
...
void PIMBlock::mad(BurstType& dstBst, BurstType& src0Bst, BurstType& src1Bst, BurstType& src2Bst)
{
    ...
    else if (pimPrecision_ == INT8)
    {
        for (int i = 0; i < 32; i++)
        {
            dstBst.u8Data_[i] = (uint8_t)((int8_t)src0Bst.u8Data_[i] * (int8_t)src1Bst.u8Data_[i] + (int8_t)src2Bst.u8Data_[i]);
        }
    }
    else
