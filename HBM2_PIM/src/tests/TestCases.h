...
class DataDim
{
  private:
    ...
    {
        uint32_t elements_per_block = getNumElementsPerBlocks();
        switch (kn_type)
        {
            case KernelType::GEMV:
            case KernelType::GEMVTREE:
            {
                ...
                weight_npbst_.loadTobShape(elements_per_block);

                input_npbst_.shape.push_back(batch_size_);
                input_npbst_.shape.push_back(input_dim_);
                input_npbst_.loadTobShape(elements_per_block);
            ...
            case KernelType::RELU:
            {
                ...
                output_npbst_.loadTobShape(elements_per_block);
