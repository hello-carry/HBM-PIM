...
// 设置内部的 PIM 控制寄存器（用于测试）
void PIMKernel::setPIMControl(bool pim_op, int crf_toggle_cond, bool grfA_zero, bool grfB_zero)
{
    setControl(&bst_hab_pim_, pim_op, crf_toggle_cond, grfA_zero, grfB_zero);
    setControl(&bst_hab_, !pim_op, crf_toggle_cond, grfA_zero, grfB_zero);
}
