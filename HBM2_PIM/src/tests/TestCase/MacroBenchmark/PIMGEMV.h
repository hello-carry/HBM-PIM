  ...
  public:
        ...
        : kernel_type_(k), batch_(b), out_(out), in_(in)
    {
        mem_ = make_shared<MultiChannelMemorySystem>("ini/HBM2_samsung_2M_16B_x64.ini",
                                                     "system_hbm.ini", ".", "example_app",
                                                     256 * 64 * 2);
        pim_mem_ = make_shared<MultiChannelMemorySystem>("ini/HBM2_samsung_2M_16B_x64.ini",
                                                         "system_hbm.ini", ".", "example_app",
                                                         256 * 64 * 2);
        // Read the channel count from the config file instead of hard-coding it
    ...
    // Get the total data size for bandwidth calculation
    uint64_t getTotalDataSize()
    {
        return dim_data_->getDataSize(dim_data_->output_dim_, dim_data_->batch_size_) +
...
class PIMBenchFixture : public testing::Test
{
  public:
    ...
    void printStats(uint64_t cycle)
    {
        ...
        if (perfTest != nullptr) {
            uint64_t total_data_bytes = perfTest->getTotalDataSize();
            double bandwidth_gb_per_sec = (double)total_data_bytes / (1024.0 * 1024.0 * 1024.0) / time_sec;

            cout << "> Bandwidth : " << bandwidth_gb_per_sec << " GB/s" << endl;
            cout << "> Data Size : " << (total_data_bytes / (1024.0 * 1024.0)) << " MB" << endl;
        }
...
// MOV bandwidth tests moved to TestCase/Bandwidth/PIMMOVGRF2Bank.h
