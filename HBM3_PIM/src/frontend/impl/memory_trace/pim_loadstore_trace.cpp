    ...
    void tick() override {
      ...
      while(!req_full && !is_finished()) {
        ...
        switch (t.req_type) {
          ...
          case 14: request_sent = m_memory_system->send({t.addr, Request::Type::PIM_VADD}); break;
          case 15: request_sent = m_memory_system->send({t.addr, Request::Type::PIM_WR_BK}); break;
          default:;
        }
        if (request_sent) {
    ...
    void init_trace(const std::string& file_path_str) {
      ...
      while (std::getline(trace_file, line)) {
        ...
        if (tokens[0] == "LD") {
          ...
          req_type = 13;
        } else if (tokens[0] == "PIM_VADD") {
          req_type = 14;
        } else if (tokens[0] == "PIM_WR_BK") {
          req_type = 15;
        } else {
          throw ConfigurationError("Trace {} format invalid!", file_path_str);
        }
