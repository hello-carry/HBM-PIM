...
class HBM3PIMController final : public IDRAMController, public Implementation {
  ...
  public:
    ...
    bool send(Request& req) override {
      // Every request needs a final_command; a request may map to multiple commands but only one final_command
      req.final_command = m_dram->m_request_translations(req.type_id);

      // Forward existing write requests to incoming read requests
      // Read-after-write optimization: if a write to the same address is pending, read the data directly from the sense amplifier (SA)
      // The pending queue holds read requests that have already issued their read command and are waiting for data to return; it only holds RD requests
      ...
      switch (req.type_id) {
        ...
        case Request::Type::PIM_VADD:       is_success = m_pim_buffer.enqueue(req);   break;
        case Request::Type::PIM_WR_BK:      is_success = m_pim_buffer.enqueue(req);   break;
  ...
  private:
    ...
    void serve_completed_reads() {
      // The pending queue holds read requests that have already issued their read command and are waiting for data to return
      if (pending.size()) {
    ...
    bool schedule_request(ReqBuffer::iterator& req_it, ReqBuffer*& req_buffer) {
      ...
      // The active buffer holds requests that are currently being served
