...
class HBM3PIM : public IDRAM, public Implementation {
  ...
  public:
    ...
    inline static const std::map<std::string, std::vector<int>> timing_presets = {
      //   name           rate      nBL  nC  nRCDRD  nRCDWR  nRP  nRA  nRC  nWR   nRTPS   nRTPL  nCWL  nCCDS  nCCDL  nCCDAB  nCCDSB  nRRDS  nRRDL  nWTRS  nWTRL  nRTW  nFAW  nRFC  nRFCSB  nREFI  nREFISB  nRREFD  tCK_ps
      ...
      // The relationship between rate and t_CK must be recomputed later, see: int tCK_ps = 1E6 / (m_timing_vals("rate") / 4); // QDR DQ pins (DQR technique is used)     m_timing_vals("tCK_ps") = tCK_ps;
   ...
   ***********************************************/
    ...
    inline static const ImplLUT m_request_translations = LUT (
      m_requests, m_commands, {
        ...
        {"pim-vadd", "VADD"}, {"pim-write-to-bank", "WR"}
      }
    );
  ...
  private:
    ...
    void set_timing_vals() {
      ...
      populate_timingcons(this, {
          ...
          /*** Vector Add ***/
          {.level = "channel", .preceding = {"VADD"}, .following = {"VADD"}, .latency = V("nCCDAB")},
          {.level = "pseudochannel", .preceding = {"VADD"}, .following = {"VADD"}, .latency = V("nBL")},
