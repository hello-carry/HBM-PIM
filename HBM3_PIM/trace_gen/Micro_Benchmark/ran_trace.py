#!/usr/bin/env python3
"""Random aggregation-bandwidth trace for HBM3-PIM.

Mapping-agnostic: emits only a plain random byte-address stream
    addr_i = base + rand_line * 32B
...
DEFAULT_SPAN_LINES = (3125 * 131072) // PREFETCH_SIZE   # same address span as the largest stride sweep
...
BASE_YAML = CONFIG_DIR / "rand_agg_bdw.yaml"
RUN_YAML = CONFIG_DIR / "rand_agg_bdw_run.yaml"   # generated per run; base yaml left untouched
RAMULATOR_BIN = CONFIG_DIR / "ramulator2"
CYCLES_RE = re.compile(r"memory_system_cycles:\s*(\d+)")


def gen_trace(n_cmd: int, span_lines: int, seed: int = 12345, base: int = 0) -> List[str]:
    """Pure random walk: addr = base + rand_line*32B over [0, span_lines).  No hierarchy awareness."""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="HBM3-PIM random bandwidth trace")
    ...
    ap.add_argument("--span-lines", type=int, default=DEFAULT_SPAN_LINES, help="random address range in 32B lines")
    ap.add_argument("--seed", type=int, default=12345)
    ap.add_argument("--tck-ns", type=float, default=0.769)
    ap.add_argument("-o", "--output", default="ran_trace.trace", help="trace filename under trace_gen/")
    ap.add_argument("--no-run-sim", dest="run_sim", action="store_false")
    ap.set_defaults(run_sim=True)
    args = ap.parse_args()

    trace_path = TRACE_DIR / args.output
    trace_path.write_text("\n".join(gen_trace(args.n_cmd, args.span_lines, args.seed)) + "\n")
    print(f"Generated random trace: {trace_path}  ({args.n_cmd} commands over {args.span_lines} lines)")
    if not args.run_sim:
        return 0
    ...
    print("------   Random Bandwidth Summary   ------")
